   ...
   while True:
      try:
         ...
         if datas[0] == "start":
            ...
            bx = int(datas[1])
            by = int(float(datas[2]))
            my_ball.move_next(bx, by)
         else:
            bat_R.setPos(datas[0])
      except:
         pass
...
my_ball = ball.Ball(table=my_table, x_speed=x_velocity, y_speed=y_velocity,
                    width=24, height=24, colour="white", x_start=288, y_start=188)

# Bat 공장으로부터 배트를 주문합니다
bat_L = bat.Bat(table=my_table, width=15, height=100, x_posn=20, y_posn=150, colour="black")
